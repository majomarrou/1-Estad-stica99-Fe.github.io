...
La tasa de mortalidad infantil es un indicador ampliamente utilizado para evaluar el estado de salud de una población y está influenciada por diversos aspectos sociales, culturales y económicos (López et al., 1998). El estudio de la mortalidad infantil proporciona información crucial sobre las principales causas de muerte en niños y su reducción ha sido planteada como un objetivo central en los "Millennium Development Goals" establecidos por las Naciones Unidas en el año 2000, con una fecha límite de 2015 (United Nations, s/f). omo tal, y frente a esos objetivos, es seguro afirmar que un niño tiene más chances de sobrevivir hasta las 5 años hoy de las que tenía en 1990 (UN IGME, 2021) Aunque se ha logrado avances significativos, existen aún brechas importantes entre países desarrollados y aquellos en desarrollo, lo que resalta la necesidad de seguir trabajando para cerrar estas disparidades.

En este contexto, el objetivo de este trabajo es identificar los factores que tienen mayor incidencia en la tasa de mortalidad infantil a nivel mundial, con el fin de brindar información valiosa a los responsables de formular políticas y diseñar intervenciones adecuadas. Para ello, se utiliza un modelo de regresión gaussiana que analiza el impacto de variables explicativas en la mortalidad infantil. Las variables consideradas en el modelo son la expectativa de vida, la desnutrición infantil, la atención prenatal y el índice de saneamiento, las cuales han sido seleccionadas debido a su alta correlación con la variable dependiente mortalidad infantil. Este análisis permitirá obtener una visión más clara de los factores que contribuyen a la mortalidad infantil y facilitará la toma de decisiones informadas para abordar este problema global de manera efectiva.Además, en este estudio se emplea un análisis de conglomerados para organizar los casos en subconjuntos o grupos, utilizando la estrategia divisiva. Esta metodología permite identificar patrones de similitud entre los casos y agruparlos en función de su similitud. 

Al combinar el modelo de regresión gaussiana con el análisis de conglomerados, se espera obtener una visión más completa y detallada de los factores que influyen en la mortalidad infantil. Esta combinación de enfoques nos permitirá no solo identificar las variables que tienen mayor incidencia en la tasa de mortalidad infantil, sino también comprender cómo interactúan entre sí y cómo se relacionan en diferentes grupos de casos. De esta manera, se proporcionará información valiosa para la formulación de políticas y la implementación de intervenciones efectivas, teniendo en cuenta las particularidades y los patrones específicos de cada grupo identificado.

# Construcción de la base de datos

La base de datos fue construida mediante la combinación manual de datos obtenidos de diversas fuentes. Principalmente, se utilizaron la página web oficial de la CIA en la sección "Guide to Country Comparisons", la base de datos del Banco Mundial y la base de datos de la Organización Mundial de la Salud. Todos los datos extraídos corresponden a la versión más actualizada disponible en cada una de estas bases de datos.
...
if (nrow(valores_influyentes) > 0) {
  ...
  diagnosticos$Resultado[diagnosticos$Diagnóstico == "Valores influyentes"] <- "No hay resultados disponibles"
}
...
Para la realización del analisis de conglomerados, se seleccionan las variables de expectativa de vida (Expvidnc), atención prenatal (atenpre) e índice de saneamiento (indsan) para nuestro análisis de conglomerados. Estas variables forman parte del modelo de correlación anterior. Además, se incluye la variable dependiente Mort y la variable Paises.
...
dianaPlot +
  geom_text_repel(data = subset(merged_data, dianaMorpoor), aes(label = name),
                  max.overlaps = 50, min.segment.length = unit(0, 'lines'),
                  color = "#333333", size = 4, fontface = "bold", family = "sans")
...
United Nations Inter-Agency Group for Child Mortality Estimation (UN IGME) (2021). Levels and trends in child mortality, consultado el 3 de Julio de 2023.
https://data.unicef.org/resources/levels-and-trends-in-child-mortality-2021/
...
León López, Rogelio, Gallegos Machado, Bárbara, Estévez Rodríguez,
Eduardo, & Rodríguez García, Sonia. (1998). Mortalidad infantil:
Análisis de un decenio. Revista Cubana de Medicina General Integral,
14(6), 606-610.
